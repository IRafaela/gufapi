...
async def post_usuario(usuario: UsuarioSchemaCreate, session: AsyncSession = Depends(get_session)):
...
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        usuario: UsuarioSchemaArtigos = result.unique.scalars().one_or_none()
...
async def put_usuario(usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        usuario_up: UsuarioSchemaBase = result.unique().scalars().one_or_none()
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).where(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        usuario_del: UsuarioSchemaArtigos = result.unique().scalars().one_or_none()
